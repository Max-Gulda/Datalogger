...
void send_data_to_phone(){
    ...
    int16_t value = (int16_t)((*(pDatabuffer + 0  ) << 8) | *(pDatabuffer + 1  ));
    bytes[1] = (uint8_t)(value & 0xFF);   // Extract low byte
    bytes[0] = (uint8_t)((value >> 8) & 0xFF);  // Extract high byte 
    write_handle_raw(rightLegHandle_ACC_X,bytes,2);
    
    value = (int16_t)((*(pDatabuffer + 2  ) << 8) | *(pDatabuffer + 3  ));
    bytes[1] = (uint8_t)(value & 0xFF);   // Extract low byte
    bytes[0] = (uint8_t)((value >> 8) & 0xFF);  // Extract high byte
    write_handle_raw(rightLegHandle_ACC_Y,bytes,2);

    value = (int16_t)((*(pDatabuffer + 4  ) << 8) | *(pDatabuffer + 5  ));
    bytes[1] = (uint8_t)(value & 0xFF);   // Extract low byte
    bytes[0] = (uint8_t)((value >> 8) & 0xFF);  // Extract high byte
    write_handle_raw(rightLegHandle_ACC_Z,bytes,2);

    value = (int16_t)((*(pDatabuffer + 8  ) << 8) | *(pDatabuffer + 9  ));
    bytes[1] = (uint8_t)(value & 0xFF);   // Extract low byte
    bytes[0] = (uint8_t)((value >> 8) & 0xFF);  // Extract high byte
    write_handle_raw(rightLegHandle_GYRO_X,bytes,2);

    value = (int16_t)((*(pDatabuffer + 10 ) << 8) | *(pDatabuffer + 11  ));
    bytes[1] = (uint8_t)(value & 0xFF);   // Extract low byte
    bytes[0] = (uint8_t)((value >> 8) & 0xFF);  // Extract high byte
    write_handle_raw(rightLegHandle_GYRO_X,bytes,2);

    value = (int16_t)((*(pDatabuffer + 12  ) << 8) | *(pDatabuffer + 13  ));
    bytes[1] = (uint8_t)(value & 0xFF);   // Extract low byte
    bytes[0] = (uint8_t)((value >> 8) & 0xFF);  // Extract high byte
    write_handle_raw(rightLegHandle_GYRO_X,bytes,2);
    
    int emg = (ADC_RDATA(ADC0));
    value = (int16_t)(emg);
    bytes[1] = (uint8_t)(value & 0xFF);   // Extract low byte
    bytes[0] = (uint8_t)((value >> 8) & 0xFF);  // Extract high byte
    write_handle_raw(rightLegHandle_EMG,bytes,2);
...
void TIMER1_IRQHandler(void){
    ...
    if(SET == timer_interrupt_flag_get(TIMER1, TIMER_INT_CH0)){
        ...
        append_int_to_string(buf, (int16_t)((*(pDatabuffer + 0  ) << 8) | *(pDatabuffer + 1  ))    );
        append_int_to_string(buf, (int16_t)((*(pDatabuffer + 2  ) << 8) | *(pDatabuffer + 3  ))    );
        append_int_to_string(buf, (int16_t)((*(pDatabuffer + 4  ) << 8) | *(pDatabuffer + 5  ))    );
        append_int_to_string(buf, (int16_t)((*(pDatabuffer + 8  ) << 8) | *(pDatabuffer + 9  ))    );
        append_int_to_string(buf, (int16_t)((*(pDatabuffer + 10 ) << 8) | *(pDatabuffer + 11 ))    );
        append_int_to_string(buf, (int16_t)((*(pDatabuffer + 12 ) << 8) | *(pDatabuffer + 13 ))    );

        buf_size = append_int_to_string(buf, ADC_RDATA(ADC0)  );
